...
namespace tau_hlt{
  ...
  namespace {
    inline float& GetCellImpl(tensorflow::Tensor& cellGridMatrix, int tau_idx, int phi_idx, int eta_idx,
                                L2TauTagNNv1::NNInputs NNInput_idx){
  ...
  private:
    ...
    template <typename VPos, typename LVec>  std::tuple<float, float, int, int> getEtaPhiIndices(const VPos& position, const LVec& tau_p4);
    template <typename LVec>  std::tuple<float, float, int, int> getEtaPhiIndices(const float& eta, const float& phi, const LVec& tau_p4);
  ...
  L2TauNNProducer::L2TauNNProducer(const edm::ParameterSet& cfg, const L2TauNNProducerCacheData* cacheData):
           ...
           for(int var_idx =0; var_idx < tensor_shape.at(3); var_idx++){
             ...
             if(std::isnan(nonstd_var)){
               ...
               if(debugLevel>2){
                 ...
                 if(var_idx+1 < tensor_shape.at(3)) std::cout << L2TauTagNNv1::varNameMap.at(static_cast<NNInputs>(var_idx+1))
                   << "\t = " <<getCell(static_cast<NNInputs>(var_idx+1)) << std::endl;
                 if(var_idx+2 < tensor_shape.at(3)) std::cout << L2TauTagNNv1::varNameMap.at(static_cast<NNInputs>(var_idx+2))
                   << "\t = " <<getCell(static_cast<NNInputs>(var_idx+2)) << std::endl;
                 if(var_idx+3 < tensor_shape.at(3)) std::cout << L2TauTagNNv1::varNameMap.at(static_cast<NNInputs>(var_idx+3))
                   << "\t = " <<getCell(static_cast<NNInputs>(var_idx+3)) << std::endl;
                 if(var_idx+4 < tensor_shape.at(3)) std::cout << L2TauTagNNv1::varNameMap.at(static_cast<NNInputs>(var_idx+4))
             ...
             for(int var_idx =0; var_idx < tensor_shape.at(3); var_idx++){
               ...
               if(getCell(NNInputs::l1Tau_pt) *256.0 == 54){
                 ...
                 if(debugLevel>4 && debugLevel<10){
                   ...
                   std::cout << "\nvar name= " << L2TauTagNNv1::varNameMap.at(static_cast<NNInputs>(var_idx)) <<"\t tau_idx = " << tau_idx<<"\t phi_idx = " << phi_idx << "\t eta_idx = " << eta_idx << " \tvalue =" << getCell(static_cast<NNInputs>(var_idx));
                 }
               }
             }
           }
   ...
   template<typename VPos, typename LVec> std::tuple<float, float, int, int> L2TauNNProducer::getEtaPhiIndices(const VPos& position, const LVec& tau_p4)
   {
   ...
   template <typename LVec> std::tuple<float, float, int, int> L2TauNNProducer::getEtaPhiIndices(const float& eta, const float& phi, const LVec& tau_p4){
      ...
      for (int eta_idx = 0; eta_idx<nCellEta ; eta_idx++){
        for (int phi_idx = 0; phi_idx<nCellPhi ; phi_idx++){
          ...
          if(getCell(NNInputs::EcalSize)>1.){
            ...
            getCell(NNInputs::EcalEnergyStdDev) = (getCell(NNInputs::EcalEnergyStdDev)  -  ( getCell(NNInputs::EcalEnergySum) * getCell(NNInputs::EcalEnergySum) ) / getCell(NNInputs::EcalSize)   ) / (getCell(NNInputs::EcalSize) -1 );
          }
          else{
          ...
          if(getCell(NNInputs::HcalSize)>1.){
            // (stdDev - (enSum*enSum)/size) / (size-1)
            getCell(NNInputs::HcalEnergyStdDev) = (getCell(NNInputs::HcalEnergyStdDev)  -  ( getCell(NNInputs::HcalEnergySum) * getCell(NNInputs::HcalEnergySum) ) / getCell(NNInputs::HcalSize)   ) / (getCell(NNInputs::HcalSize) -1 );
          }
          else{
     ...
     for (int tau_idx = 0; tau_idx< nTaus; tau_idx++){
       ...
       for (const auto it : TrackGood) {
          ...
          //  float dphi = reco::deltaPhi(patatrackPhi,tauPhi);
            ...
            std::tie(deta, dphi, eta_idx, phi_idx) = getEtaPhiIndices(patatrackEta, patatrackPhi, allTaus[tau_idx]->polarP4());
        ...
        for (int eta_idx = 0; eta_idx<nCellEta ; eta_idx++){
          for (int phi_idx = 0; phi_idx<nCellPhi ; phi_idx++){
            ...
            if(getCell(NNInputs::PatatrackPtSum)>0.){
              getCell(NNInputs::PatatrackDeltaEta) = getCell(NNInputs::PatatrackDeltaEta) / getCell(NNInputs::PatatrackPtSum);
              getCell(NNInputs::PatatrackDeltaPhi) = getCell(NNInputs::PatatrackDeltaPhi) / getCell(NNInputs::PatatrackPtSum);
              getCell(NNInputs::PatatrackChi2OverNdof) = getCell(NNInputs::PatatrackChi2OverNdof) / getCell(NNInputs::PatatrackPtSum);
   ...
   void L2TauNNProducer::produce(edm::Event& event, const edm::EventSetup& eventsetup){
     ...
     const int evt_id = event.id().event();
     ...
     for(size_t inp_idx = 0; inp_idx<L1TauDesc_.size(); inp_idx++){
       ...
       for (size_t tau_pos = 0; tau_pos < nTau; ++tau_pos) {
          ...
          if(debugLevel_ > 0){
            std::cout << evt_id << " \t " << (allTaus[static_cast<int>(tau_idx)])->polarP4().pt() << " \t " << tau_score.at(static_cast<int>(tau_idx)) << std::endl;
          }
          (*tau_tags.get())[tau_pos] = tau_score.at(static_cast<int>(tau_idx));
